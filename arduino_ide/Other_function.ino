...
void Enter_and_convert_values() // Берем значения с АЦП и преобразуем в ШИМ данные для машинки 
{
  ...
  FOR_3 sens_val[i] = map(analogRead(PIN_SENS[i]), 1800, 500, 0, 255);  // Преобрзование значений с одного диапозона в другой
  FOR_3 sens_val[i] = constrain(sens_val[i], 0, 255);                   // Ограничение значений в заданом диапозоне (0 - 255)
  if (sens_val[0] < 135) sens_val[0] = 0;      
  if (sens_val[1] < 190) sens_val[1] = 0;                         // Задаем предельное значение шим сигнала
  if (sens_val[2] < 135) sens_val[2] = 0;
